...
func NewChatPeer(ip string, sourcePort int, readFrom chan MessageToSend, writeFunc OnMessageReceived) (*ChatPeer, error) {
	...
	chatPeer := ChatPeer{ip, sourcePort, readFrom, writeFunc, host, make(map[peer.ID]network.Stream)}

	// Set a function as stream handler.
	// This function is called when a peer connects, and starts a stream with this protocol.
	host.SetStreamHandler("/chat/1.0.0", chatPeer.handleStream)
...
func (p *ChatPeer) ConnectTo(dest string) error {
	...
	s, err := p.p2pHost.NewStream(context.Background(), info.ID, "/chat/1.0.0")
	if err != nil {
		return err
	}
...
func (p *ChatPeer) Close() {
	p.p2pHost.RemoveStreamHandler("/chat/1.0.0")
	p.p2pHost.Close()
}
